...
export default function FeedItem({ item }: { item: FeedItemType }) {
    ...
    useEffect(() => {
        ...
        checkOwner();
    }, [item.user_id, supabase]);

    const handleDelete = async () => {
        if (!confirm("Bunu silmek istediğine emin misin?")) return;

        try {
            const table = item.type === 'story' ? 'stories' : item.type === 'ad' ? 'ads' : 'pets';
            ...
            <div className="p-4 flex items-center justify-between">
                ...
                {isOwner && (
                    <button
                        ...
                        className="ml-2 p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors"
                        title="Sil"
                    >
